...
```
 ...
 # 2. 创建开机自启服务（推荐 systemd 方式）
 sudo tee /etc/systemd/system/force-time-sync.service << 'EOF'
 [Unit]
 ...
 [Service]
 Type=oneshot
 ExecStartPre=/bin/sleep 2  # 等待网络稳定
 ExecStart=/usr/sbin/ntpdate -u ntp.ubuntu.com || /usr/sbin/ntpdate -u pool.ntp.org
 ExecStartPost=/sbin/hwclock -w  # 将系统时间写入硬件（非必需，但可维持本次运行期间时间）
...
```
WIFI_SSID="ravvo_tennis_0x1a71"
MASTER_MAC="40:55:48:a5:41:cc"
SLAVE_MAC="68:24:99:a2:e2:d4"
```

```powershell
# 1) 结束会占用 DLL 的进程（会关掉所有资源管理器窗口）
Stop-Process -Name explorer -Force -ErrorAction SilentlyContinue
Get-Process python -ErrorAction SilentlyContinue | Stop-Process -Force

# 2) 删掉扩展目录（网盘本体一般不在这里；删的是用户目录下的 shell 扩展）
Remove-Item -LiteralPath "$env:APPDATA\baidu" -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item -LiteralPath "$env:LOCALAPPDATA\baidu" -Recurse -Force -ErrorAction SilentlyContinue

# 3) 重启资源管理器
Start-Process explorer

# 4) 确认不再加载（应提示“没有正在运行的任务匹配指定标准”）
