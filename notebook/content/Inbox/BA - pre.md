...
One common example is using a simplified replacement policy, typically the LRU algorithm,  instead of the pseudo-LRU scheme that is actually implemented in hardware.

Inter-core interference, which can occur in multi-threaded systems, means that different threads may affect each other when they share the same caches. this kind of interactions are usually not simulated and they are also difficult to observe in real hardware.  
...
The last simplification I want to mention is using a fully associative cache instead of a set-associative cache. In my work, I mainly focus on analysing the impact of this assumption.  
...
On the right side there is a two-way set-associative cache. Here, the cache space is divided into four different sets, and each set contains two slots, and this is what “two-way” means.
...
Set-associative caches are employed in most modern processors. Fully associative caches are very expensive, so they are only used for some small hardware components such as TLBs.
...
i'm going to emphasize capacity misses and conflict misses, because they are most relevant in my thesis.

a capacity miss means the required block has been replaced due to the limited capacity and new coming data, so you can no longer find it in the cache  
  
a conflict miss occurs only in set-associative cache, it is a little bit more complicate.   
 
Now in this example, the 2 places in set 2 are already occupied. If we now want to access a new block 10, it can only go to set 2 on the right side, because 10 module 4 is 2.  

Then the least recently used block in set 2, will be replaced, even if there are other blank spaces in the cache. So block 2 gets out, block 10 comes in.
...
The algorithm is based on the reuse distance theorem by mattson et al., this is a famous metric for modeling cache behavior. And the method to measure cache misses under this framework was proposed by Kim et al.
...
And then, i predicted cache misses with the two algorithm and compare the results with actual cache misses.
...
I also ran the experiments under different thread configurations, both single thread and multiple threads, in order to study the effect of parallelism. And as the workload for the SpMV program, i used 40 matrices from Sparsesuite, which is a well known collection of sparse matrices.
...
If a new data block comes in, the first step is to decide which set it should be placed in. Then the following procedure in this set is similar to what happens in the fully associative cache.

---
To evaluate the prediction accuracy of both algorithms, we need to know the real number of cache misses.

The machine we used supports hardware performance counters, which can record low-level events such as cache misses .

So I wrote a simple program that runs the real SpMV computation, well this is how SpMV looks like in CSR format using cplusplus, and I use LIKWID marker APIs to enable hardware counters during the execution, so that i can measure cache misses within this area.

The data collected by hardware counters is highly reliable, so it can be treated as ground truth.  The results from this program will be compared with the results predicted by our simulation algorithms later.

---

I calculated the Mean Absolute Percentage Error, or MAPE between predicted results and ground truth for every matrix, with both original and the extended algorithm. This value represents how far the prediction results are to the real data, 

As shown in the tables, the extended algorithm produces prediction results that are closer to the ground truth, which means a smaller mean of MAPE value.  
