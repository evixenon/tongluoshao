...
| **容器**         | **实现原理**             | **随机访问效率**               | **查找速度**                 | **插入速度**                 | **删除速度**                 | **排序速度**     | **使用场景建议**                   |
| -------------- | -------------------- | ------------------------ | ------------------------ | ------------------------ | ------------------------ | ------------ | ---------------------------- |
| `vector`       | **动态数组**             | ✅ **O(1)**               | `O(n)`                   | 尾部：`O(1)`  <br>其他：`O(n)` | 尾部：`O(1)`  <br>其他：`O(n)` | `O(n log n)` | 需要快速随机访问或尾部操作（如动态数组、缓存数据）。   |
| `deque`        | **分段连续数组**           | ✅ **O(1)**（略慢于 `vector`） | `O(n)`                   | 头尾：`O(1)`  <br>中间：`O(n)` | 头尾：`O(1)`  <br>中间：`O(n)` | `O(n log n)` | 需要高效头尾插入/删除（如任务队列、滑动窗口）。     |
| `list`         | **双向链表**             | ❌ **O(n)**               | `O(n)`                   | 任意位置：`O(1)`              | 任意位置：`O(1)`              | `O(n log n)` | 频繁中间插入/删除（如事件处理器链表）。         |
| `forward_list` | **单向链表**             | ❌ **O(n)**               | `O(n)`                   | 任意位置：`O(1)`              | 需前驱节点：`O(1)`             | `O(n log n)` | 内存敏感的单向操作（如嵌入式系统、轻量级链表）。     |
| `queue`        | **适配器（默认基于`deque`）** | ❌ **不支持**                | 不支持                      | 队尾：`O(1)`                | 队首：`O(1)`                | 不支持          | FIFO 场景（如消息队列、BFS 遍历）。       |
| `stack`        | **适配器（默认基于`deque`）** | ❌ **不支持**                | 不支持                      | 栈顶：`O(1)`                | 栈顶：`O(1)`                | 不支持          | LIFO 场景（如函数调用栈、撤销操作）。        |
| priority_queue | **堆（默认基于`vector`）**  | ❌ **不支持**                | 不支持                      | `O(log n)`               | `O(log n)`               | 无需（自动按优先级排序） | 按优先级处理元素（如任务调度、Dijkstra 算法）。 |
| `set`          | **红黑树**              | ❌ **不支持**（需按键遍历）         | `O(log n)`               | `O(log n)`               | `O(log n)`               | 无需（自动有序）     | 需要唯一元素且自动排序（如字典、有序集合）。       |
| `multiset`     | **红黑树**              | ❌ **不支持**（需按键遍历）         | `O(log n)`               | `O(log n)`               | `O(log n)`               | 无需（自动有序）     | 允许重复元素且需排序（如统计频率）。           |
| `map`          | **红黑树**              | ❌ **不支持**（需按键遍历）         | `O(log n)`（按键）           | `O(log n)`               | `O(log n)`               | 无需（按键自动排序）   | 键值对存储且按键有序（如配置项、数据库索引）。      |
| `multimap`     | **红黑树**              | ❌ **不支持**（需按键遍历）         | `O(log n)`（按键）           | `O(log n)`               | `O(log n)`               | 无需（按键自动排序）   | 允许重复键且需范围查询（如学生成绩按班级分类）。     |
| `uno_set`      | **哈希表**              | ❌ **不支持**（无序）            | 平均 `O(1)`  <br>最坏 `O(n)` | 平均 `O(1)`  <br>最坏 `O(n)` | 平均 `O(1)`  <br>最坏 `O(n)` | `O(n log n)` | 快速去重且无需顺序（如黑名单过滤、缓存键集合）。     |
| un_mulset      | **哈希表**              | ❌ **不支持**（无序）            | 平均 `O(1)`  <br>最坏 `O(n)` | 平均 `O(1)`  <br>最坏 `O(n)` | 平均 `O(1)`  <br>最坏 `O(n)` | `O(n log n)` | 允许重复元素且无序（如词频统计）。            |
| uno_map        | **哈希表**              | ❌ **不支持**（无序）            | 平均 `O(1)`  <br>最坏 `O(n)` | 平均 `O(1)`  <br>最坏 `O(n)` | 平均 `O(1)`  <br>最坏 `O(n)` | `O(n log n)` | 快速键值查找且无需顺序（如缓存、键值数据库）。      |
| un_mulmap      | **哈希表**              | ❌ **不支持**（无序）            | 平均 `O(1)`  <br>最坏 `O(n)` | 平均 `O(1)`  <br>最坏 `O(n)` | 平均 `O(1)`  <br>最坏 `O(n)` | `O(n log n)` | 允许重复键且无序（如日志按时间戳分组）。         |
| `array`        | **固定数组**             | ✅ **O(1)**               | `O(n)`                   | 不支持                      | 不支持                      | `O(n log n)` | 固定大小的高性能需求（如预定义常量表、数学向量运算）。  |
| `bitset`       | **位数组**              | ✅ **O(1)**（按位索引）         | `O(1)`（按位检查）             | `O(1)`（按位设置）             | `O(1)`（按位清除）             | 不支持          | 高效位操作（如权限管理、状态压缩、布隆过滤器）。     |
